...
先考虑求第20小的数。
1. 先在A里二分在B里定位，假设A中<=a[mid]的数有甲个，然后在B中找到<=a[mid]的位置，算出来前面有乙个，这样总共定位出来了tmp=甲+乙个小于等于A[mid]的数
...
1. A和B的元素都是偶数的情况。假设A=[a1,a2,a3,a4], B=[b1,b2,b3,b4]
    ...
    b. 如果a2!=b2: a2>b2, a2<b2，只讨论a2>b2: 
        ...
        iv) 将[a1,a2], [b3,b4]看成新的数组，他们两个数组中的第2小就是整体数组中的第4小。递归求解。证明：
            ...
            假设b3是第2小的，说明a1<=b3<=a2<=b4，对应到原数组，比a1大, b3前面就有3个数了，刚好是第4小。
...
3. 17(长数组长度)<k<=27(和)。假设求第23小
    ...
    b. B数组中，b13>=a10、b14>=a9..时都是有可能的, 即[b13...b17]。淘汰掉[b1..b12]共12个数
...
public:
    ...
    int get_kth_num(vector<int>& A, vector<int>& B, int k) {
        ...
        if (k<=s) { //k<=s的情况，短数组所有数都有可能。长短数组各取k个
