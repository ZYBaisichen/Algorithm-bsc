...
2. 2个数，有1对
3. 3个数，两两相邻，共3对
4. n个数，且n个数不重。一定能找到最大值max和次大值。任何位置x，顺时针一定找到可以找到第一个比x大的数y，同样在逆时针方向上可以找到第一个比x大的数z。
...
3. 当遇到比栈顶大的元素，栈顶弹出时。假设栈顶是3个4，在下面是2个5，任何一个4在逆时针和顺时针方向上都能找到一个5，结算3*2=6对。3个4内部两两可见，结算c(3,2)=3对。共结算9对
4. 假设从max遍历之后，栈中还有数，就需要来到清算阶段。比如{5,4,3,2}，则栈中{{2,1},{3,1},{4,1},{5,1}}。
   a. 当栈大小>2时：栈顶元素为{2,k}，则顺时针和逆时针方向每个2都可以找到一对，然后2内部可以找到c(k,2)对
   b. 栈大小刚好等于2时：栈底max只有一个呢，{{4,k},{5,1}}，顺时针或者逆时针方向遇到的是同一个数，则对外产生k*1对，内部产生c(k,2)对；
