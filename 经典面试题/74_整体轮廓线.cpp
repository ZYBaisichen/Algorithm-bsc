...
arr[0]表示大楼的左边界，arr[1]表示大楼的右边界，arr[2]表示大楼的高度(一定大于0)。没做大楼的地基都在x轴上，大楼之间可能会有重叠，请返回整体的轮廓线数组。
...
2. 为防止同一个位置多次降低或增加, 还需要增加一个map，记录高度出现的次数。以为所有key中，可能需要知道当前最大的高度是什么，所以需要使用到有序表
    ...
    vector<vector<int>> getSkyline(vector<vector<int>>& buildings) {
        ...
        for (int i=0;i<ops.size();i++) {
            ...
            if (max_h_times.empty()) {
                ...
                max_height[ops[i].x] = max_h_times.rbegin()->first;
        ...
        for (auto& iter : max_height) {
            ...
            if (cur_h == last_h) { //没找到高度跳变点则不猴急答案
                continue;
            }
            vector<int> tmp(2,0);
            tmp[0] = cur_x;
