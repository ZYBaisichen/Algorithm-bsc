...
/*
矩阵如果是n*n， 则有：
    O(n^4)个长方形：任意点一个点有N^2个可能，任意两个点形成长方形，因为有重复，所以数量级打到了N^4个长方形
...
public:
    int largest1BorderedSquare(vector<vector<int>> &grid)
    {
        ...
        // 遍历每个正方形，假设点到了(i，j)和(a,b)两点，当前点的左边和右边连续1的数量是否超过了正方形边长；右上点下方1的数量；做下点左边1的数量；一次检查都满足正方形边长的话则返回满足条件
        int n = grid.size();
        if (n == 0)
        {
            return 0;
