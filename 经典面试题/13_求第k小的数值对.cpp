...
public:
    ...
    /*    
    1. 首先让nums有序
    2. 先定位第一个数。如果需要56个数，总数N=10，那么第一个数肯定是第5个数(因为第一个数搞定[1,10)，第二个数搞定[20,30), 第5个数搞定[50,60))
        a. 第一个数位于的位置是：a=(K-1)/10
    3. 然后定位第二个数。假设和第一个数arr[a]相等的数有b个，比第一个数arr[a]小的数有c个。
        a. 比第一个数小的数能搞定的数值对有：c*N个，则还剩rest=k-c*N个数需要搞定
        b. 这rest里，从头开始，每个数可以搞定b个数值对(比如: 3,1 3,2 3,2 )， 所以第rest个数值对在nums的(rest-1)/b位置
    //
    */
    ...
    */
    ...
    /*    
        基于方法3，进一步的，将pivot的选择更稳定一些，采用bfprt算法。感觉有点鸡肋，工程上数据量大的时候概率选肯定很稳定，数据量小的时候稳不稳定无所谓。
    //
    */
