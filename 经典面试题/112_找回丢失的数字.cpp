...
2. 当最右侧数字是0时，则需要f(n-1,1,2)+f(n-1,2,2)+...+f(n-1,200,2)。挨个将n-1位置的数字变成1到200(最大限制)，求出来让0..n-1范围上数组达标的方法数
...
public:
    ...
    */
    ...
    int restoreWays_dp(vector<int>& arr) {
        ...
        //dp[i][v][s] 在i位置数是v，右边数和当前数关系是s的情况下，0...i范围上的达标数组可能性数量
        vector<vector<vector<int>>> dp(len, vector<vector<int>>(201, vector<int>(3, 0)));
        ...
        for (int i=1;i<len;i++) {
            ...
            for (int v=1;v<201;v++) {
                for (int s=0;s<3;s++) {
                    int ways=0;
                    if (s==0 || s==1) { //当右边的数比我大或等于时，左边可以随意变
                        for (int pre=1;pre<201;pre++) { //左边变成1到200，都试一遍。每次调用需要计算pre和当前的v的关系
