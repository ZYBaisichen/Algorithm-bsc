...
1. A和B的元素都是偶数的情况。假设A=[a1,a2,a3,a4], B=[b1,b2,b3,b4]
    ...
    b. 如果a2!=b2: a2>b2, a2<b2，只讨论a2>b2: 
        i)  因为a3>=a2>b2， 所以前半段数在a3前面，a3最多是5小，第所以a3,a4不可能是上中位数
        ii) 因为b2<a2<=a3，同时b2<=b3<=b4，所以b1,b2一定在前面，b2最多是第3小，不可能是上中数。
...
2. A和B长度都是奇数时：假设A=[a1,a2,a3,a4,a5], B=[b1,b2,b3,b4,b5]
    ...
    b. 如果a3>b3:
        i) 因为a3>b3>=b2>=b1，又因为a3>=a2>=a1，所以a3最多是第6个数，所以[a3,a4,a5]都不可能。
...
2. 10(短数组长度)<k<=17(长数组长度)，假设要求第15小.
    ...
    c. b中b5到b15是可能的，[b5...b15]有11个数，A[a1..a10]有10个数。所以单独判断b5>=a10，如果满足，则b5就是第15小。如果不满足则淘汰b5。
       将[a1..a10]和[b6...b15]组合求上中位数(第10小)，因为前面已经淘汰掉了[b1...b5]，所以刚好该上中位数就是第15小
...
public:
    ...
    int get_kth_num(vector<int>& A, vector<int>& B, int k) {
        ...
        if (k>l) { //l<k<=l+s
            /*
            淘汰掉: shorts[1...k-l-1], longs[1..k-s-1] （10,17,取23，淘汰shorts[1...5], long[1..12]）
            保留：shorts[k-l...s], longs[k-s...l] (保留shorts[6..10], longs[13..17]， 5个数对5个数，第5小+前面淘汰的12+5=第22小，差一个，所以要单独看shorts[6]和longs[13])
            手动判断第k-l和k-s个数是否是第k小 (如果shorts[6]和longs[13]都不是，则shorts[7..10]和longs[14...17]是4对4，第4小加上前面淘汰的6+13=19个数，刚好是第23小)
            */
           if(shorts[k-l-1] >= longs[l-1]) { //(k-l)+l=k 看shorts里的第[k-l]
               return shorts[k-l-1];
           }
           if (longs[k-s-1] >= shorts[s-1]) { //k-s+s = k 看longs里的第[k-s]
                return longs[k-s-1];
           }
            //同样元素的数组获取大小
    ...
    */
    double findMedianSortedArrays1(vector<int>& nums1, vector<int>& nums2) {
        ...
        if (all_len %2 == 0) { //总长时偶数
            int left = get_kth_num(nums1, nums2, all_len/2); //上中位数
            int right = get_kth_num(nums1, nums2, all_len/2+1); //下中位数
            cout << "left:" << left << " right:" << right << " all_len:" << all_len << endl;

            return (left+right)/2.0;
        } else {
            return get_kth_num(nums1, nums2, all_len/2+1); //中位数
        }
